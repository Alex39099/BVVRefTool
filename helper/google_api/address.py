...
from helper.google_api.exceptions import IncorrectCellLabel, InvalidArgumentValue
...
class Address:
    ...
    def _label_to_coordinates(self, label):
        ...
        if not m or (not self.allow_non_single and not (row and col)):
            raise IncorrectCellLabel(f'Not a valid cell label format: {label}.')
        return row, col

    def __repr__(self):
        return f'<{self.__class__.__name__} {self.label!s}>'
...
class GridRange:
    ...
    def __init__(self, label=None, start=None, end=None, worksheet_title=None,
    ...
    def create(data, wks=None):
        ...
        if isinstance(data, GridRange):
            ...
            grange = GridRange(label=data)
        elif isinstance(data, (tuple, list)):
            if len(data) < 2: raise InvalidArgumentValue("start and end required")
            grange = GridRange(start=data[0], end=data[1])
        elif isinstance(data, dict):
            grange = GridRange(propertiesjson=data)
        else:
